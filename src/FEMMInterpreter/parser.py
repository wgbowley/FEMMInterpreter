...
class ParseLines:
    ...
    def parse(cls, lines: list[str]) -> dict:
        ...
        while state.index < len(lines):
            ...
            if is_section:
                ...
                # Checks for section value
                raw_value = cls._extract_section_value(line)
                value = Deserialize.cast(raw_value)

                state.content[name] = {}
                if value: state.content[name]["value"] = value
    ...
    @classmethod
    def _extract_section_value(cls, line: str) -> str:
        """ Extract section or subsection values """
        equal_sign = line.find("=")
        if equal_sign != -1:
            # Returns the value if true
            return line[equal_sign+1:]
        return ""
