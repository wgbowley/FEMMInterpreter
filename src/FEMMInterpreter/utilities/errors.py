...
class FileTypeNotSupport(Exception):
    """ Exception for file type not supported """
    def __init__(self, file_type: str):
        """ Returns a file type not supported error """
        msg = f"Failed to parse {file_type!r} as not in supported list {FILE_TYPES!r}"
        super().__init__(msg)
...
class FormatNotSupported(Exception):
    ...
    def __init__(self, file_format: str):
        ...
        msg = f"Failed to parse {file_format!r} as not in supported list {FILE_FORMATS!r}"
        super().__init__(msg)
